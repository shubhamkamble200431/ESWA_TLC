...
def generate(model, tokenizer, sources, batch_size, max_length, num_beams, device):
    ...
    for i in range(0, len(sources), batch_size):
        ...
        gen_kw = dict(max_length=max_length, num_beams=num_beams, early_stopping=True)
        if forced_bos:
...
def main():
    parser = argparse.ArgumentParser(description="mBART50 SAN→HIN inference and evaluation")
    ...
    tokenizer = MBart50TokenizerFast.from_pretrained(args.checkpoint, src_lang=SRC_LANG, tgt_lang=TGT_LANG)
    ...
    print(f"\n{'='*60}")
    print(f"  mBART50  |  split={args.split}")
    ...
    with open(os.path.join(out_dir, f"mBART50_{args.split}_metrics.json"), "w") as f:
        json.dump(metrics, f, indent=2, ensure_ascii=False)
    with open(os.path.join(out_dir, f"mBART50_{args.split}_predictions.hi"), "w") as f:
        f.write("\n".join(preds) + "\n")
